...
@Controller('api/v2/kafeels')
@UseGuards(AuthGuard, RolesGuard)
@UseInterceptors(LoggingInterceptor, ClassSerializerInterceptor)
@ApiBearerAuth()
@ApiTags('kafeels')
export class KafeelExtendedController {
  ...
  async getKafeelByUser(@Req() req: Request): Promise<KafeelDTO> {
    return await this.kafeelService.findByFields({where:{user:req.user}});
  }
